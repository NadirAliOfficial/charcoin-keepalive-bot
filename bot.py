...
INPUT_MINT = os.getenv("INPUT_MINT", "So11111111111111111111111111111111111111112")  # wSOL
PUBLIC_KEY = os.getenv("PUBLIC_KEY", "")
WALLET_SECRET_B58 = os.getenv("WALLET_SECRET_B58", "")
RPC_URL = os.getenv("RPC_URL", "https://api.mainnet-beta.solana.com")

MICRO_BUY_USD = float(os.getenv("MICRO_BUY_USD", "0.50"))
FALLBACK_BUY_USD = float(os.getenv("FALLBACK_BUY_USD", "1.00"))
...
def activity_ok() -> bool:
    ...
    return not has_no_trades_last_x(window)
...
def ensure_wallet_ready():
    ...
    print(f"Wallet balance: {bal/1_000_000_000:.6f} SOL")
...
def build_and_send_swap(input_amount_raw: int) -> str:
    ...
    if DRY_RUN:
        print(f"[DRY RUN] Would swap {input_amount_raw} units → CHAR")
        return "dryrun-sig"
    ...
    # Sign & send using solders
# --- Sign & send using solders (with Presigner) ---
    from solana.rpc.api import Client
    from solders.keypair import Keypair
    from solders.pubkey import Pubkey
    ...
    msg = tx.message                         # VersionedMessage (Legacy or V0)

    # Get canonical message bytes to sign
    msg_bytes = to_bytes_versioned(msg)

    # Make a presigner from your key and the computed signature
    kp  = Keypair.from_bytes(b58decode(WALLET_SECRET_B58))
    sig = kp.sign_message(msg_bytes)         # solders.signature.Signature
    pk  = kp.pubkey()
    presigner = Presigner(pk, sig)

    # IMPORTANT: this constructor expects (message, signers)
...
def can_spend_usd(amount: float) -> bool:
    ...
    return (spent + amount) <= MAX_DAILY_USD
...
def main_loop():
    print(f"CharCoin keep-alive bot | TEST_MODE={TEST_MODE} DRY_RUN={DRY_RUN} FORCE_BUY_NOW={FORCE_BUY_NOW}")
    print(f"Using bucket: {'m5 (~5m test)' if TEST_MODE else 'h24 (prod)'} | Interval={CHECK_INTERVAL_SECONDS}s")
    ensure_wallet_ready()

    if FORCE_BUY_NOW:
        try:
            sig = do_keepalive_once(MICRO_BUY_USD)
            print(f"[FORCE_BUY_NOW] ok, tx sig: {sig}")
        except Exception as e:
            print(f"[FORCE_BUY_NOW] ERROR: {e}")
        return

    while True:
        try:
            if activity_ok():
                print(f"[{datetime.now()}] Activity OK — no buy.")
            else:
                print(f"[{datetime.now()}] Inactive — trying ${MICRO_BUY_USD:.2f}")
                try:
                    sig = do_keepalive_once(MICRO_BUY_USD)
                    print(f"[BUY] success {sig}")
                except Exception as e:
                    print(f"[BUY] primary failed: {e}")
                    if MICRO_BUY_USD < FALLBACK_BUY_USD and can_spend_usd(FALLBACK_BUY_USD):
                        try:
                            print(f"[BUY] retrying fallback ${FALLBACK_BUY_USD:.2f}")
                            sig = do_keepalive_once(FALLBACK_BUY_USD)
                            print(f"[BUY] fallback success {sig}")
                        except Exception as e2:
                            print(f"[BUY] fallback failed: {e2}")
        except Exception as e:
